...
(async () => {
  ...
  const T = {
    ...
    adventuresN: es ? 'Adventures' : 'Adventures',
    pdf:        es ? 'Descargar la baraja' : 'Download full deck',
    pdfLabel:   es ? 'La baraja completa' : 'The whole deck',
    pdfNote:    es
      ? 'Las mismas tarjetas impresas, en un PDF que se abre de corrido. En inglés, como vienen de imprenta.'
      : 'The same printed cards in one PDF you can page through. English, as printed.',
    pages:      es ? 'páginas' : 'pages'
  };

  /* Peso en MB con una decimal: el líder puede estar con datos y 4 MB no es
     lo mismo que 400 KB. Se muestra ANTES de tocar, no después. */
  const mb = bytes => `${(bytes / 1e6).toFixed(1)} MB`;

  /* El grado se volvió bilingüe el 20-ago. Se acepta también el string suelto
     de antes: el SW sirve el JSON cacheado mientras revalida, así que la
     primera carga después de publicar todavía trae la forma vieja. */
  const grado = g => (typeof g === 'string' ? g : Card.t(g, lang));
  ...
  try {
    ...
    // El arte del estuche es el fondo de la cabecera, no un bloque aparte.
    const tinte = (!adv && deck.art) ? deck.art : null;

    /* Sobre el arte va SOLO la miga de pan (pedido 20-ago). El eyebrow y la
       descripción bajan al cuerpo de la página:
       - el hero se lee como la portada del mazo y no como una tarjeta con
         texto encima;
       - y de paso desaparece el problema de contraste que obligaba a tapar
         media portada con velo para que la descripción fuera legible sobre
         la mascota. Con solo la miga arriba, el velo puede ser mínimo. */
    const intro = tinte ? '' : Card.t(deck.intro, lang);

    Shell.mountHeader(document.getElementById('pagehead'), {
      eyebrow: adv ? `${deckName} · ${Card.categoria(adv, lang)}`
                   : (tinte ? null : `Cub Scouts · ${T.decks}`),
      title: adv ? Card.t(adv.name, lang) : deckName,
      sub: adv ? Card.t(adv.summary, lang) : intro,
    ...
    /* ---------- nivel 1 · el rank ---------- */
    // El arte lo pinta la cabecera. Acá arranca el contenido: el rótulo y la
    // descripción que antes iban encima del arte, y después el aviso.
    document.getElementById('head').innerHTML = `
      ${tinte ? `
        <p class="page__label" style="margin-top:var(--space-5)">Cub Scouts · ${T.decks}</p>
        ${Card.t(deck.intro, lang)
          ? `<p class="deckintro">${Card.t(deck.intro, lang)}</p>` : ''}` : ''}
      ${(deck.translation === 'pendiente' && deck.cards.length)
        ? Shell.notice({ icono: 'traducir', html: T.pending }) : ''}`;
    ...
    /* Los datos duros de la baraja: van pegados al banner, como el pie de una
       ficha de producto. Sin el estuche: el banner ya muestra el arte, y una
       segunda imagen de lo mismo era decirlo dos veces. */
    const esLiderVista = API.getMode() === 'lider';
    const pdf = esLiderVista ? deck.pdf : null;
    const facts = `
      <p class="deckfacts rise" style="--i:0">
        ${grado(deck.grade) ? `<span>${grado(deck.grade)}</span>` : ''}
        <span>${activities.length} ${T.cards}</span>
        <span>${deck.adventures.length} ${T.adventuresN}</span>
      </p>`;

    /* La descarga va DESPUÉS de las Adventures (pedido 20-ago): el líder entra
       a elegir la actividad de la semana, no a bajarse la baraja entera. Lo
       segundo es la salida para cuando lo primero no le alcanza, y una salida
       va al final del camino, no en la puerta. */
    const descarga = pdf ? `
      <section class="rise" style="--i:3; margin-top:var(--space-7)">
        <p class="page__label">${T.pdfLabel}</p>
        <a class="btn btn--primary" href="${pdf.src}" download>
          ${T.pdf} · ${mb(pdf.bytes)}
        </a>
        <p class="deckfacts__note">${pdf.pages} ${T.pages} · ${T.pdfNote}</p>
      </section>` : '';

    document.getElementById('content').innerHTML = `
      ${facts}
      ${API.getMode() !== 'lider' ? `
      <section class="rise" style="--i:1; margin-top:var(--space-5)">
      ...
      <section class="rise" style="--i:4; margin-top:var(--space-7)">
