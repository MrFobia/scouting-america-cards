  ...
  try {
    ...
    if (!filas.length) {
      cont.innerHTML = Shell.estado({
        ...
        accion: `<a class="btn btn--primary" href="/site/barajas.html">${es ? 'Ver las barajas' : 'See the decks'}</a>`
      });
      return;
    }
    cont.innerHTML = filas.map(({ d, p }) => `
      <div class="progresscard rise-io" style="margin-bottom:var(--space-4)">
        ...
        <p style="margin:0">
          <a class="link-arrow" href="/site/baraja.html?d=${d.id}&lang=${lang}">${es ? 'Seguir' : 'Continue'} →</a>
        </p>
      </div>`).join('');
