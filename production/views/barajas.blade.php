...
(async () => {
  ...
  /** Grado escolar de una baraja, tolerando el string suelto de antes. */
  const grado = g => (typeof g === 'string' ? g : (g && (g[lang] || g.es)) || '');
  ...
  try {
    ...
    /* La ficha de cada baraja es su PORTADA (`d.art`), no una foto. Antes iba
       una foto del brandbook por baraja: correctas, pero intercambiables —
       ninguna decía cuál baraja era, y la grilla entera se veía igual de azul.
       La portada sí identifica, y hace juego con la interna, que abre con ella.

       La foto queda de respaldo para la baraja de planeación del pack, que no
       tiene estuche impreso. */
    ...
    cont.innerHTML = visibles.map(d => {
      ...
      const estilo = art
        ? ` style="--deck-ink:${art.ink}; --deck-on:${art.on}"` : '';
      return `<li>
        <a class="tile ${clase}"${estilo} href="/preview/947?d=${d.id}&lang=${lang}">
          ...
          <span>
            ...
            <span class="tile__label">${(() => {
              ...
              // El grado es bilingüe desde el 20-ago; la baraja de planeación
              // no tiene grado porque no es de un rank. Se aceptan las dos
              // formas a propósito: el SW sirve el índice cacheado mientras
              // revalida, así que en la primera carga después de publicar el
              // teléfono todavía tiene el `grade` viejo, que era un string.
              // Sin esto esa carga pintaba "undefined" debajo de cada baraja.
              if (!r) return grado(d.grade) || (es ? 'Para líderes' : 'For leaders');
